...
import { api } from './api'
import { PagedList } from './tmpTypes'
...
const randomInteger = (from: number, to: number): number => Math.floor(random() * (to - from + 1) + from)

const randomItem = <T>(arr: T[]): T => {
  return arr[Math.floor(random() * arr.length)]
}

const byId = <T extends {id: number | string}>(list: T[], id: number | string): T => {
...
const createList = <T>(createItem: (index: number, count: number) => T, countFrom = 10, countTo = countFrom): T[] => {
  const count = randomInteger(countFrom, countTo)
...
const getPage = <T>(list: T[], query: QueryParameters = {}): PagedList<T> => {
  const limit = getQueryParmater<number>(query.count)
  const skip = getQueryParmater<number>(query.skip)
  return {
    limit,
    skip,
    total: list.length,
...
const filterList = <T>(list: T[], query: QueryParameters = {}): T[] => {
