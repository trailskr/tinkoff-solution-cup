...
const resolvePath = (pathToResolve: string): string => pathResolve(process.cwd(), pathToResolve)

const forEachFileInDirectoryRecursively = (dirAbsolutePath: string, extensionsRe: RegExp, onFile: (fileName: string) => void): void => {
...
const getResponses = (responses: ApiResponses, from: number, to: number): [status: number, response: ApiResponse][] => {
...
export const generateApi = (swaggerUrl: string, options: ApiGenerationOptions = {}): Promise<ApiGenerationResult> => {
  ...
  const apiPromises = Object.entries(swaggerUrls).map(([apiName, apiUrlOrFilePath]) => {
    ...
    const fetchUrl = (url: string): Promise<Api> => fetch(apiUrlOrFilePath, {
    ...
    const loadFile = (filePath: string): Promise<Api> => new Promise<Api>((resolve, reject) => {
  ...
  return Promise.all(apiPromises).then((apiList) => {
    ...
    const mapCallerName = (name: string): string => {
